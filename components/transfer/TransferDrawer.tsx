...
export function TransferDrawer({
	...
	return (
		<Drawer open={open} onOpenChange={setOpen}>
			<DrawerContent className='max-h-[85vh] rounded-t-3xl px-2 pb-4 bg-gray-100'>
				<div className='mx-auto w-full px-4 py-2'>
					<DrawerHeader>
						<DrawerTitle className='text-sm text-gray-700 text-center'>
							Security Verify
						</DrawerTitle>
					</DrawerHeader>
				</div>
				<div className='grid items-start gap-4  '>
					<div className='grid gap-2'>
						<Label
							htmlFor='email'
							className={`${oldPinError ? 'text-red-500' : ''} ml-1`}
						>
							Enter Your Pin
						</Label>
						<Input
							...
							className={`${
								oldPinError ? 'border-red-500 text-red-500' : 'border-gray-300'
							}`}
						/>
						{oldPinError && (
